...
if RobloxReplicatedStorage:FindFirstChild("boykisser-clan") then return end

local boykisser-clanContainer = Instance.new("Folder", RobloxReplicatedStorage)
boykisser-clanContainer.Name = "boykisser-clan"
local objectPointerContainer, scriptsContainer = Instance.new("Folder", boykisser-clanContainer), Instance.new("Folder", boykisser-clanContainer)
...
table.freeze(boykisser-clan.about)
...
local libs = {
	{
		['name'] = "HashLib",
		['url'] = "https://rizve.us.to/boykisser-clan/hash"
	},
	{
		['name'] = "lz4",
		['url'] = "https://rizve.us.to/boykisser-clan/lz4"
	},
	{
		['name'] = "DrawingLib",
		['url'] = "https://rizve.us.to/boykisser-clan/drawing"
	}
}

if script.Name == "VRNavigation" then
	StarterGui:SetCore("SendNotification", {
		Title = "[boykisser-clan]",
...
function Bridge:InternalRequest(body, timeout)
	...
	if success and result then
		error("BOYKISSER-CLAN SERVER ERROR: " .. tostring(result), 2)
	end
...
function Bridge:loadstring(source, chunkName)
	...
	coreModule.Parent = boykisser-clanContainer
	...
	if result then
		...
		while task.wait() do
			...
			if (tick() - clock > 5) then
				warn("[BOYKISSER-CLAN]: loadstring failed and timed out")
				for _, module in pairs(cachedModules) do
					module:Destroy()
				end
...
function Bridge:request(options)
	...
	return {
		Success = false,
		StatusMessage = "Can't connect to boykisser-clan web server: " .. self.serverUrl,
		StatusCode = 599;
...
boykisser-clan.boykisser-clan = {
	...
	HttpSpy = function(state)
		...
		boykisser-clan.rconsoleinfo("Http Spy is set to '" .. tostring(state) .. "'")
		httpSpy = state
	end,
}

function boykisser-clan.boykisser-clan.get_real_address(instance)
...
function boykisser-clan.boykisser-clan.spoof_instance(instance, newinstance)
	...
	do
		if type(newinstance) == "number" then 
			...
			newAddress = boykisser-clan.boykisser-clan.get_real_address(newinstance)
		end
	end
	local objectValue = Instance.new("ObjectValue", objectPointerContainer)
	objectValue.Name = HttpService:GenerateGUID(false)
...
function boykisser-clan.boykisser-clan.GetGlobal(global_name)
...
function boykisser-clan.boykisser-clan.SetGlobal(global_name, value)
...
function boykisser-clan.boykisser-clan.Compile(source)
...
function boykisser-clan.require(moduleScript)
...
function boykisser-clan.loadstring(source, chunkName)
...
function boykisser-clan.request(options)
	...
	if httpSpy then
		boykisser-clan.rconsoleprint("-----------------[boykisser-clan Http Spy]---------------\nUrl: " .. options.Url .. 
	...
	options.Headers["User-Agent"] = options.Headers["User-Agent"] or "boykisser-clan/Roblox-Executor/" .. tostring(boykisser-clan.about._version)
	options.Headers["Exploit-Guid"] = tostring(hwid)
	options.Headers["boykisser-clan-Fingerprint"] = tostring(hwid)
...
boykisser-clan.http = {request = boykisser-clan.request}
boykisser-clan.http_request = boykisser-clan.request

function boykisser-clan.HttpGet(url, returnRaw)
	assert(type(url) == "string", "invalid argument #1 to 'HttpGet' (string expected, got " .. type(url) .. ") ", 2)
	local returnRaw = returnRaw or true

	local result = boykisser-clan.request({
		Url = url,
		Method = "GET"
	})
...
function boykisser-clan.GetObjects(asset)
	return {
		InsertService:LoadLocalAsset(asset)
	}
end

boykisser-clan.game = newproxy(true)
local gameProxy = getmetatable(boykisser-clan.game)

gameProxy.__index = function(self, index)
	if index == "HttpGet" or index == "HttpGetAsync" then
		return function(self, ...)
			return boykisser-clan.HttpGet(...)
		end
	elseif index == "HttpPost" or index == "HttpPostAsync" then
		return function(self, ...)
			return boykisser-clan.HttpPost(...)
		end
	elseif index == "GetObjects" then
		return function(self, ...)
			return boykisser-clan.GetObjects(...)
		end
	end
...
boykisser-clan.Game = boykisser-clan.game

function boykisser-clan.getgenv()
	return boykisser-clan
end
...
function boykisser-clan.readfile(path)
...
function boykisser-clan.writefile(path, content)
...
function boykisser-clan.appendfile(path, content)
	...
	boykisser-clan.writefile(path, readVal .. content)
end
function boykisser-clan.loadfile(path)
	assert(type(path) == "string", "invalid argument #1 to 'loadfile' (string expected, got " .. type(path) .. ") ", 2)
	return boykisser-clan.loadstring(boykisser-clan.readfile(path))
end
boykisser-clan.dofile = boykisser-clan.loadfile
function boykisser-clan.isfolder(path)
...
function boykisser-clan.isfile(path) -- return not boykisser-clan.isfolder(path)
...
function boykisser-clan.listfiles(path)
...
function boykisser-clan.makefolder(path)
...
function boykisser-clan.delfolder(path)
...
function boykisser-clan.delfile(path)
...
function boykisser-clan.getcustomasset(path)
...
boykisser-clan.base64 = base64
boykisser-clan.base64_encode = base64.encode
boykisser-clan.base64encode = base64.encode
boykisser-clan.base64_decode = base64.decode
boykisser-clan.base64decode = base64.decode

boykisser-clan.crypt = {
	base64 = base64,
...
boykisser-clan.crypt.generatebytes = function(len)
	return boykisser-clan.crypt.generatekey(len)
end
boykisser-clan.crypt.random = function(len)
	return boykisser-clan.crypt.generatekey(len)
end
boykisser-clan.crypt.decrypt = boykisser-clan.crypt.encrypt

function boykisser-clan.crypt.hash(txt, hashName)
...
boykisser-clan.hash = boykisser-clan.crypt.hash

boykisser-clan.crypt.lz4 = lz4
boykisser-clan.crypt.lz4compress = lz4.compress
boykisser-clan.crypt.lz4decompress = lz4.decompress

boykisser-clan.lz4 = lz4
boykisser-clan.lz4compress = lz4.compress
boykisser-clan.lz4decompress = lz4.decompress

local Drawing, drawingFunctions = DrawingLib.Drawing, DrawingLib.functions
boykisser-clan.Drawing = Drawing

for name, func in drawingFunctions do
	boykisser-clan[name] = func
end

-- / Miscellaneous \ --
function boykisser-clan.getproperties(instance)
...
boykisser-clan.gethiddenproperties = boykisser-clan.getproperties

local _saveinstance = nil
function boykisser-clan.saveinstance(options)
	options = options or {}
	...
	_saveinstance = _saveinstance or boykisser-clan.loadstring(boykisser-clan.HttpGet("https://raw.githubusercontent.com/luau/SynSaveInstance/main/saveinstance.luau", true), "saveinstance")()
	return _saveinstance(options)
end
boykisser-clan.savegame = boykisser-clan.saveinstance

function boykisser-clan.getexecutorname()
	return boykisser-clan.about._name
end
function boykisser-clan.getexecutorversion()
	return boykisser-clan.about._version
end

function boykisser-clan.identifyexecutor()
	return boykisser-clan.getexecutorname(), boykisser-clan.getexecutorversion()
end
boykisser-clan.whatexecutor = boykisser-clan.identifyexecutor

function boykisser-clan.get_hwid()
	return hwid
end
boykisser-clan.gethwid = boykisser-clan.get_hwid

function boykisser-clan.getscriptbytecode(script_instance)
...
boykisser-clan.dumpstring = boykisser-clan.getscriptbytecode
...
local function konst_call(konstantType: string, scriptPath: Script | ModuleScript | LocalScript): string
	local success: boolean, bytecode: string = pcall(boykisser-clan.getscriptbytecode, scriptPath)
	...
	local httpResult = boykisser-clan.request({
...
function boykisser-clan.Decompile(script_instance)
...
boykisser-clan.decompile = boykisser-clan.Decompile

-- for some reason, UniversalSynSaveInstance is using the Disassemble function the same as Decompile.
function boykisser-clan.__Disassemble(script_instance)
	if typeof(script_instance) ~= "Instance" then
...
boykisser-clan.__disassemble = boykisser-clan.__Disassemble

function boykisser-clan.queue_on_teleport(source)
...
boykisser-clan.queueonteleport = boykisser-clan.queue_on_teleport

function boykisser-clan.setclipboard(content)
...
boykisser-clan.toclipboard = boykisser-clan.setclipboard

function boykisser-clan.rconsoleclear()
	return Bridge:rconsole("cls")
end
boykisser-clan.consoleclear = boykisser-clan.rconsoleclear

function boykisser-clan.rconsolecreate()
	return Bridge:rconsole("crt")
end
boykisser-clan.consolecreate = boykisser-clan.rconsolecreate

function boykisser-clan.rconsoledestroy()
	return Bridge:rconsole("dst")
end
boykisser-clan.consoledestroy = boykisser-clan.rconsoledestroy

function boykisser-clan.rconsoleprint(...)
	local text = ""
	for _, v in {...} do
...
boykisser-clan.consoleprint = boykisser-clan.rconsoleprint

function boykisser-clan.rconsoleinfo(...)
	local text = ""
	for _, v in {...} do
		text = text .. tostring(v) .. " "
	end
...
boykisser-clan.consoleinfo = boykisser-clan.rconsoleinfo

function boykisser-clan.rconsolewarn(...)
	local text = ""
	for _, v in {...} do
		text = text .. tostring(v) .. " "
	end
...
boykisser-clan.consolewarn = boykisser-clan.rconsolewarn

function boykisser-clan.rconsolesettitle(text)
...
boykisser-clan.rconsolename = boykisser-clan.rconsolesettitle
boykisser-clan.consolesettitle = boykisser-clan.rconsolesettitle
boykisser-clan.consolename = boykisser-clan.rconsolesettitle

function boykisser-clan.clonefunction(func)
...
function boykisser-clan.islclosure(func)
...
function boykisser-clan.fireclickdetector(part)
...
function boykisser-clan.firetouchinterest(toucher, toTouch, touch_state)
	...
	local toTouchAddress = tostring(boykisser-clan.boykisser-clan.get_real_address(toTouch))
	...
	if touch_state == 0 then
		...
		boykisser-clan.boykisser-clan.spoof_instance(newPart, toTouch)
	...
	elseif touch_state == 1 then
		...
		boykisser-clan.boykisser-clan.spoof_instance(toTouch, tonumber(toTouchAddress))
...
function boykisser-clan.fireproximityprompt(proximityprompt, amount, skip)
...
function boykisser-clan.setsimulationradius(newRadius, newMaxRadius)
...
function boykisser-clan.isreadonly(t)
...
function boykisser-clan.rconsoleinput(text)
	task.wait()
	return "N/A"
end
boykisser-clan.consoleinput = boykisser-clan.rconsoleinput
...
function boykisser-clan.getrenv()
	return renv
end

function boykisser-clan.isexecutorclosure(func)
	assert(type(func) == "function", "invalid argument #1 to 'isexecutorclosure' (function expected, got " .. type(func) .. ") ", 2)
	for _, genv in boykisser-clan.getgenv() do
		if genv == func then
			return true
		end
	end
	...
	if check(boykisser-clan.getgenv().getrenv()) then
		return false
	end
	return true
end
boykisser-clan.checkclosure = boykisser-clan.isexecutorclosure
boykisser-clan.isourclosure = boykisser-clan.isexecutorclosure
...
function boykisser-clan.isrbxactive()
	return windowActive
end
boykisser-clan.isgameactive = boykisser-clan.isrbxactive
boykisser-clan.iswindowactive = boykisser-clan.isrbxactive

function boykisser-clan.getinstances()
	return workspace.Parent:GetDescendants()
end

local nilinstances, cache = {Instance.new("Part")}, {cached = {}}

function boykisser-clan.getnilinstances()
	return nilinstances
end
...
boykisser-clan.cache = cache

function boykisser-clan.getgc()
...
function boykisser-clan.getrunningscripts()
	local scripts = {}
	for _, v in pairs(boykisser-clan.getinstances()) do
		if v:IsA("LocalScript") and v.Enabled then table.insert(scripts, v) end
	end
	return scripts
end
boykisser-clan.getscripts = boykisser-clan.getrunningscripts

function boykisser-clan.getloadedmodules()
	local modules = {}
	for _, v in pairs(boykisser-clan.getinstances()) do
...
function boykisser-clan.checkcaller()
	local info = debug.info(boykisser-clan.getgenv, 'slnaf')
	return debug.info(1, 'slnaf')==info
end

function boykisser-clan.getthreadcontext()
	return 3
end
boykisser-clan.getthreadidentity = boykisser-clan.getthreadcontext
boykisser-clan.getidentity = boykisser-clan.getthreadcontext

function boykisser-clan.setthreadidentity()
	return 3, "Not Implemented"
end
boykisser-clan.setidentity = boykisser-clan.setthreadidentity
boykisser-clan.setthreadcontext = boykisser-clan.setthreadidentity

function boykisser-clan.getsenv(script_instance)
...
function boykisser-clan.getscripthash(instance) -- !
...
function boykisser-clan.getconnections()
	return {{
		Enabled = true, 
		ForeignState = false, 
		LuaConnection = true, 
...
function boykisser-clan.hookfunction(func, rep)
	local env = getfenv(debug.info(2, 'f'))
...
boykisser-clan.replaceclosure = boykisser-clan.hookfunction

function boykisser-clan.cloneref(reference)
...
function boykisser-clan.compareinstances(x, y)
...
function boykisser-clan.gethui()
	return boykisser-clan.cloneref(workspace.Parent:FindService("CoreGui"))
end

function boykisser-clan.isnetworkowner(part)
...
function boykisser-clan.deepclone(object) -- used for initialization
	local lookup = {}
...
boykisser-clan.debug = table.clone(debug) -- the debug funcs was not by me (.rizve) credits goes to the person that made it
function boykisser-clan.debug.getinfo(f, options)
...
function boykisser-clan.debug.getmetatable(table_or_userdata)
...
boykisser-clan.debug.setmetatable = setmetatable

function boykisser-clan.getrawmetatable(object)
	...
	local raw_mt = boykisser-clan.debug.getmetatable(object)
	if raw_mt and raw_mt.__metatable then
		raw_mt.__metatable = nil 
		local result_mt = boykisser-clan.debug.getmetatable(object)
...
function boykisser-clan.setrawmetatable(object, newmetatbl)
...
function boykisser-clan.hookmetamethod(t, index, func)
	...
	local o = t
	local mt = boykisser-clan.debug.getmetatable(t)
	mt[index] = func
	t = mt
	return o
end
...
function boykisser-clan.setfpscap(cap)
	cap = tonumber(cap)
...
function boykisser-clan.getfpscap()
	return fpscap
end

function boykisser-clan.mouse1click(x, y)
	x = x or 0
	y = y or 0
...
function boykisser-clan.mouse1press(x, y)
	x = x or 0
	y = y or 0
...
function boykisser-clan.mouse1release(x, y)
	x = x or 0
	y = y or 0
...
function boykisser-clan.mouse2click(x, y)
	x = x or 0
	y = y or 0
...
function boykisser-clan.mouse2press(x, y)
	x = x or 0
	y = y or 0
...
function boykisser-clan.mouse2release(x, y)
	x = x or 0
	y = y or 0
...
function boykisser-clan.mousescroll(x, y, z)
	VirtualInputManager:SendMouseWheelEvent(x or 0, y or 0, z or false, workspace.Parent)
end

function boykisser-clan.mousemoverel(x, y)
	x = x or 0
	y = y or 0
...
function boykisser-clan.mousemoveabs(x, y)
	x = x or 0
	y = y or 0
...
function boykisser-clan.getscriptclosure(s)
	return function()
		return table.clone(boykisser-clan.require(s))
	end
end
boykisser-clan.getscriptfunction = boykisser-clan.getscriptclosure

function boykisser-clan.isscriptable(object, property)
...
task.spawn(function() -- queue_on_teleport handler
	...
	if type(source) == "string" and source ~= "" then
		...
		setfenv(rawLoadstringFunc, merge(getfenv(rawLoadstringFunc), boykisser-clan))
...
local function listen(coreModule)
	while task.wait() do
		...
		if type(execution_table) == "table" and execution_table["x e n o"] and (not execution_table.__executed) and coreModule.Parent == scriptsContainer then
			...
			setfenv(execLoad, merge(getfenv(execLoad), boykisser-clan))
			task.spawn(execLoad)
